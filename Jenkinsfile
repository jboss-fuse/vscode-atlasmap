...
node('rhel8'){
	...
	if(params.UPLOAD_LOCATION) {
		stage('Snapshot') {
			...
			sh "sftp -C ${UPLOAD_LOCATION}/snapshots/vscode-atlasmap/ <<< \$'put -p -r ${filesToPush[0].path}'"
            stash name:'vsix', includes:filesToPush[0].path
            def tgzFilesToPush = findFiles(glob: '**.tgz')
            stash name:'tgz', includes:tgzFilesToPush[0].path
            sh "sftp -C ${UPLOAD_LOCATION}/snapshots/vscode-atlasmap/ <<< \$'put -p -r ${tgzFilesToPush[0].path}'"
		}
    }
}
...
node('rhel8'){
	if(publishToMarketPlace.equals('true')){
		timeout(time:5, unit:'DAYS') {
			input message:'Approve deployment?', submitter: 'apupier,lheinema,joshiraez,tsedmik,djelinek,jpav'
		}
		...
		stage("Publish to Marketplace") {
            ...
            def vsix = findFiles(glob: '**.vsix')
            sh "sftp -C ${UPLOAD_LOCATION}/stable/vscode-atlasmap/ <<< \$'put -p -r ${vsix[0].path}'"
            def tgz = findFiles(glob: '**.tgz')
            sh "sftp -C ${UPLOAD_LOCATION}/stable/vscode-atlasmap/ <<< \$'put -p -r ${tgz[0].path}'"
